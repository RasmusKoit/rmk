//! Common data types used in RMK protocol
//!
//! This module defines the basic data structures used across endpoints.
//! Many types are re-exported from the main rmk crate to maintain compatibility.

use postcard_schema::Schema;
use serde::{Deserialize, Serialize};

/// Empty request type for endpoints that don't need request data
...
/// Key position in the keyboard matrix
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize, Schema)]
...
/// Serializable action type for postcard-rpc protocol
/// This represents the full keyboard action, not just a 16-bit keycode
...
pub enum Action {
    ...
    /// A normal key stroke
    Key(KeyCode),
    /// Key stroke with modifier combination
    KeyWithModifier(KeyCode, u8), // simplified modifier as u8
    /// Activate a layer
    LayerOn(u8),
    /// Activate a layer with modifier combination
    LayerOnWithModifier(u8, u8),
    /// Deactivate a layer
    LayerOff(u8),
    /// Toggle a layer
    LayerToggle(u8),
    /// Set default layer
    DefaultLayer(u8),
    /// Activate a layer and deactivate all other layers
    LayerToggleOnly(u8),
    /// Trigger macro by index
    TriggerMacro(u8),
    /// Oneshot layer
    OneShotLayer(u8),
    /// Oneshot modifier
...
/// Version information (used for both protocol and firmware versions)
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize, Schema)]
pub struct Version {
...
/// Current RMK postcard-rpc protocol version
...
/// Keyboard information (includes system and keyboard configuration)
...
/// Matrix state for debugging
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize, Schema)]
pub struct MatrixState {
    /// Raw matrix state data
    /// Each bit represents a key: 1 = pressed, 0 = released
    /// Fixed size array supporting up to 256 keys (32 bytes * 8 bits)
    pub data: [u8; 32],
}
